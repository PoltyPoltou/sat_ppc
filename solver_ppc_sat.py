    ...
    for w in range(weeks):
        for g in range(groups):
            schedule[w][g] = sgp_sat.add_set_var(
                sgp_csp.schedule[w][g].lb,
                sgp_csp.schedule[w][g].ub)
    return schedule
        ...
        for g in range(sgp_csp.groups):
            for golfer in range(sgp_csp.n):
                if golfer in sgp_csp.schedule[w][g].lb:
                    assumptions.append(
                        pure_sat_model.var_encoding[w][g][golfer])
...
def solve_sgp_ppc_pure_sat(model: model.Model, propag=None, sgp_csp: model.Sgp = None, tree: TreeNode = Tree(), depth=0, pure_sat_model=None):
    if pure_sat_model == None:
        pure_sat_model = SGP_pure_sat(
            sgp_csp.groups, sgp_csp.size, sgp_csp.weeks)
    ...
    if depth == 5:
        ...
        if propagate_infos[0]:
            ...
            for memento in modifs:
                ...
                if solve_sgp_ppc_pure_sat(model, propag, sgp_csp, tree.add_child(name=str(memento)), depth=depth+1, pure_sat_model=pure_sat_model):
                    return True
                propag.backtrack()
                memento.revert()
        else:
...
def solve_sgp_ppc_sat(model: model.Model, propag=None, sgp_csp: model.Sgp = None, tree: TreeNode = Tree(), depth=0):
    if depth == 5:
        # we switch to sat modeling and solve it
        ...
        if propagate_infos[0]:
            ...
            for memento in modifs:
                memento.apply()
                propag.add_level_of_modification([])
                if solve_sgp_ppc_sat(model, propag, sgp_csp, tree.add_child(name=str(memento)), depth=depth+1):
                    return True
                propag.backtrack()
                memento.revert()
        else:
...
def solve_csp_set_sat_mix():
    return lambda g, s, w, f=None, name=None: solve_csp_sat(g, s, w, f, name, solve_method=solve_sgp_ppc_sat)


def solve_csp_pure_sat_mix():
    return lambda g, s, w, f=None, name=None: solve_csp_sat(g, s, w, f, name, solve_method=solve_sgp_ppc_pure_sat)
