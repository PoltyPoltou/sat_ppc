...
class Set_Sat_Adv(Set_Sat):
    ...
    def model_to_set_solution(self, model):
        ...
        idx_list = [0] + model
        solution_set = [
            list(sorted(self.lbounds[i])) for i in range(len(self.set_sat_idx))
        ]
    ...
    def intersection(self, i, j, k, left_right=True, right_left=True):
        ...
        for elmt in self.ubounds[i] & self.ubounds[j]:
            if left_right:
                self.add_clever_clause(
                    [(i, elmt, False), (j, elmt, False), (k, elmt, True)]
                )
            if right_left:
                self.add_clever_clause([(i, elmt, True), (k, elmt, False)])
                self.add_clever_clause([(j, elmt, True), (k, elmt, False)])
    ...
    def intersection_empty(self, i, j):
        ...
        for common_elmt in self.ubounds[i] & self.ubounds[j]:
            self.add_clever_clause([(i, common_elmt, False), (j, common_elmt, False)])
    ...
    def cardinal_lb(self, i, lb):
        ...
        if lb > len(self.lbounds[i]):
            for c in self.at_most_k(
                len(self.set_sat_idx[i]) - lb + len(self.lbounds[i]), negated_lst
            ):
                self.add_clause(c)
    ...
    def cardinal_ub(self, i, ub):
        ...
        if ub - len(self.lbounds[i]) >= 0:
            for c in self.at_most_k(
                ub - len(self.lbounds[i]), list(self.set_sat_idx[i].values())
            ):
                self.add_clause(c)
        ...
        if len(self.set_sat_idx[i]) != 0:
            ...
            if n != 0:
                ...
                if elmt_lb != None:
                    ...
                    self.add_clause(
                        [-latch_map[key_list_ordered[0]]] + list(latch_map.values())
                    )
                    self.add_clause(
                        [
                            -array_idx[key_list_ordered[0]],
                            latch_map[key_list_ordered[0]],
                        ]
                    )
                for i in range(1, n):
                    if elmt_lb == None:
