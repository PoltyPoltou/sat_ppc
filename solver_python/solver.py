...
def iterate_var_val_order_bound(model: Model, ub=True, descending=False):
    # upgrade 5 ?
    ordered_var = sorted(
        model.enum_variables(),
        key=lambda var: len(var.ub if ub else var.lb),
        reverse=descending,
    )
...
def iterate_var_val_ub(model: Model):
    priority_var = model.var_priority_dict[max(model.var_priority_dict.keys())]
    var = min(
        priority_var,
        key=lambda v: len(v.ub - v.lb) if not v.defined() else inf,
        default=None,
    )
    treillis = var.ub - var.lb
    if len(treillis) == 0:
        var = min(
            model.enum_variables(),
            key=lambda v: len(v.ub - v.lb) if not v.defined() else inf,
...
def go_to_next_node(
    exec_node: CSPNode, propagator: Propagator, backtrack=False, backjump=None
):
    if backjump != None:
        pass
    elif backtrack:
        if exec_node.up == None:
            ...
            while (
                exec_node.up != None
                and exec_node.up.children.index(exec_node)
                == len(exec_node.up.children) - 1
            ):
                propagator.backtrack()
...
def solve_iterative(model: Model, sgp=None):
    ...
    while exec_node != None:
        propagate_infos = propagator.propagate()
        new_modifs = iterate_var_val_ub(model)
        if len(new_modifs) == 0 and model.model_truth():
            ...
            exec_node = go_to_next_node(exec_node, propagator, backtrack=True)
        else:
