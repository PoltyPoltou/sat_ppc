...
class SGP_pure_sat:
    ...
    def get_set_solution(self):
        if self.solver.status:
            schedule = [[[] for j in range(self.groups)] for i in range(self.weeks)]
            for var in self.solver.get_model()[: self.idx_last_encoding_var - 1]:
                if var > 0:
                    ...
                    w = (idx - golfeur - g * self.n_golfers) // (
                        self.n_golfers * self.groups
                    )
        ...
        for w in range(sgp_pb.weeks):
            ...
            for g1 in range(sgp_pb.groups):
                for g2 in range(g1 + 1, sgp_pb.groups):
                    clause = [
                        -sgp_pb.var_encoding[w][g1][i],
                        -sgp_pb.var_encoding[w][g2][i],
                    ]
                    sgp_pb.add_clause(clause)
                        ...
                        if i1 != i2:
                            for g1 in range(sgp_pb.groups):
                                for g2 in range(sgp_pb.groups):
                                    clause = [
                                        -sgp_pb.var_encoding[w1][g1][i1],
                                        -sgp_pb.var_encoding[w2][g2][i1],
                                        -sgp_pb.var_encoding[w1][g1][i2],
                                        -sgp_pb.var_encoding[w2][g2][i2],
                                    ]
                                    sgp_pb.add_clause(clause)
            ...
            for i1 in range(sgp_pb.n_golfers):
                for i2 in range(sgp_pb.n_golfers):
                    if i1 != i2:
                        sgp_pb.add_clause(
                            [
                                -sgp_pb.var_encoding[w][g][i1],
                                -sgp_pb.var_encoding[w][g][i2],
                                encounter[w][g][i1][i2],
                            ]
                        )
                        sgp_pb.add_clause(
                            [sgp_pb.var_encoding[w][g][i1], -encounter[w][g][i1][i2]]
                        )
                        sgp_pb.add_clause(
                            [sgp_pb.var_encoding[w][g][i2], -encounter[w][g][i1][i2]]
                        )
...
def group_size_fixed_binomial_encoding(sgp_pb: SGP_pure_sat):
    ...
    all_combin = list(itertools.combinations(range(sgp_pb.n_golfers), sgp_pb.size + 1))
    for w in range(sgp_pb.weeks):
        for g in range(sgp_pb.groups):
...
def order_weeks(sgp_pb: SGP_pure_sat):
    ...
    for w in range(sgp_pb.weeks):
        for i in range(sgp_pb.n_golfers):
            sgp_pb.add_clause([-week_max[w][i], sgp_pb.var_encoding[w][0][i]])
    # at least 1 and at most 1
    for w in range(sgp_pb.weeks):
        sgp_pb.add_clause(week_max[w])
        for i1 in range(sgp_pb.n_golfers):
            for i2 in range(i1 + 1, sgp_pb.n_golfers):
                sgp_pb.add_clause([-week_max[w][i1], -week_max[w][i2]])

    # max[w,i] => !x[w,0,i'] for all i' > i
    for w in range(sgp_pb.weeks):
        for i1 in range(sgp_pb.n_golfers):
            for i2 in range(i1 + 1, sgp_pb.n_golfers):
                sgp_pb.add_clause([-week_max[w][i1], -sgp_pb.var_encoding[w][0][i2]])
    ...
    for w1 in range(sgp_pb.weeks):
        for w2 in range(w1 + 1, sgp_pb.weeks):
            for i1 in range(sgp_pb.n_golfers):
                for i2 in range(i1):
                    sgp_pb.add_clause([-week_max[w1][i1], -week_max[w2][i2]])
...
def order_group_latch(sgp_pb: SGP_pure_sat):
    ...
    for w in range(sgp_pb.weeks):
        ...
        for g in range(sgp_pb.groups):
            latch_group_list[w].append(latch(sgp_pb, sgp_pb.var_encoding[w][g]))
...
def order_week_latch(sgp_pb: SGP_pure_sat):
    latch_week_list = []
    for w in range(sgp_pb.weeks):
        latch_week_list.append(latch(sgp_pb, sgp_pb.var_encoding[w][0][::-1]))

    for w in range(1, sgp_pb.weeks):
        for i in range(len(latch_week_list[w])):
            sgp_pb.add_clause([-latch_week_list[w - 1][i], latch_week_list[w][i]])
...
def solve(groups, size, weeks, f=sys.stdout):
    ...
    print(
        "\n{}-{}-{}\nvariables : {} clauses : {}".format(
            groups, size, weeks, pb.idx_next_var - 1, len(pb.cnf.clauses)
        ),
        file=f,
    )
    print("model : {:.2f}s".format(end_model - start_model), end=" ", file=f)
    start = time()
    pb.solve()
    end = time()
